...
function onLucky(options) {
  waitFirst(selectorLucky, settingData.luckyDur, settingData.luckyTime, (node) => {
    if (running == false) {
      options.fail({
...
function onKai(options) {
  waitFirst(selectorKai, settingData.kaiDur, settingData.kaiTime, (node) => {
    if (running == false) {
      options.fail({
        msg: 'running is false'
      })
...
function onKaiBack(options) {
  waitFirst(selectorKaiBack, settingData.kaiBackDur, settingData.kaiBackTime, (node) => {
    if (running == false) {
      options.fail({
        msg: 'running is false'
      })
...
function onClose(options) {
  waitFirst(selectorClose, settingData.closeDur, settingData.closeTime, (node) => {
    if (running == false) {
      options.fail({
        msg: 'running is false'
      })
...
function onDelPress(options) {
  waitFirst(selectorLucky, settingData.delPressDur, settingData.delPressTime, (node) => {
    if (running == false) {
      options.fail({
        msg: 'running is false'
      })
...
function onDel(options) {
  waitFirst(selectorDel, settingData.delDur, settingData.delTime, (node) => {
    if (running == false) {
      options.fail({
        msg: 'running is false'
      })
...
function onDelSure(options) {
  waitFirst(selectorDelSure, settingData.delSureDur, settingData.delSureTime, (node) => {
    if (running == false) {
      options.fail({
        msg: 'running is false'
      })
...
function onBack(options) {
  waitFirst(selectorBack, settingData.backDur, settingData.backTime, (node) => {
    if (running == false) {
      options.fail({
        msg: 'running is false'
      })
...
function onTop(options) {
  waitFirst(selectorTop, settingData.topDur, settingData.topTime, (node) => {
    if (running == false) {
      options.fail({
        msg: 'running is false'
      })
...
function startError(err) {
  if (typeof err == 'string') {
    ...
    if (err && err.msg != 'running is false') {
      ...
      $thread.stopAllLoop()
      stop()
    } else if (err && err.msg == 'running is false') {
    } else {
      $log.err('未知异常[2]已暂停')
      toast('未知异常[2]已暂停')
      menuRun.icon('ic_run')
...
function createFab() {
  ...
  if (!$arc.has(fabName)) {
    ...
    menuLog.click(() => {
      ...
      $log.delete()
      sleep(1000)
      $log.info('使用日志悬浮窗时，请调整大小并拖动到右上角，不要遮挡点击操作')
